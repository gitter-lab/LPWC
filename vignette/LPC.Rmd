---
title: "LPC: Lag Penalized Correlation for Clustering Short Time Series"
...
Lag Penalized Correlation(LPC) is a distance based clustering permiting lags using weighted correlation. 
LPC accomodates lags but due to the length of the time series, the lags are penalized using gausssian kernel. 
Unlike many other time series data, LPC also accounts for irregular time sampling.


If you use LPC in published research, please cite:


If after reading through this vignette you have questions or problems
using LPC, please post them to https://support.bioconductor.org and tag "LPC".
...
library(devtools)
devtools::install_github("gitter-lab/LPC")
```
...
**Data :**  Input to LPC is a matrix. The expression matix shoud be n -by- p where n is the number of genes (in rows) and p is the timepoints where data where collected. 
...
**Data** and **Timepoints** are the main requirement to run LPC, however, there are other parameters that is used in running LPC. These are not required, but they do affect the results of LPC
...
LPC will compute similarity/correlation between genes and return three output: correlation of genes in dist form (see as.dist), lags used for each gene and the C used in the LPC. 


An example output of LPC for 10 genes using high penalty


```{r}

LPC::corr.bestlag(simdata[49:58, ], timepoints = timepoints, max.lag = 2, penalty = "high", iter = 10)
```
...
```{r clust1}
dist <- 1 - LPC::corr.bestlag(simdata[11:20, ], timepoints = timepoints, max.lag = 2, penalty = "low", iter = 10)$corr
plot(hclust(dist))
```
...
We have benefited in the development of LPC from the help and
...
- How to speed up LPC for low penalty with large gene set?
```{r, eval = FALSE}
