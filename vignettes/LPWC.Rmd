...
knitr::kable(t(data.frame(Gene1 = row1, Gene2 = row1)), align = 'c')
...
row1 <- c(0, 5, 15, 30, 45, 60)
row2 <- c(5, 15, 30, 45, 60, 75)

knitr::kable(t(data.frame(Gene1 = row2, Gene2 = row1)), align = 'c')
...
An example of the effects of applying different lags to genes 1 and 2. 
The time series plot shows aligned expression vectors and the table shows the aligned timepoints.
With no lags, $X_1 = 0$ and $X_2 = 0$, the temporal profiles of genes 1 and 2 are not aligned so the gene pair will have a low LPWC similarity score (top row).
With lags $X_1 = -1$ and $X_2 = 0$, the patterns are aligned, and the LPWC similarity score will be high (middle row).
Finally, with $X_1 = -1$ and $X_2 = 1$, the temporal shapes are once again not aligned, and the LPWC similarity score will be even lower than in the no lag case because the penalty for introducing lags is applied (bottom row).
